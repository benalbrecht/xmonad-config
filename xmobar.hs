...
Config {
    font = "xft:DejaVu Sans Mono:antialias=true:size=10",
    bgColor = "#002b36",
    fgColor = "#657b83",
	...
	Run BatteryP ["BAT0"] ["-t", "<acstatus><left>% (<timeleft>)", "-L", "10", "-H", "80", "-p", "2", 
        ...
        Run Network "eth0" ["-t","Net: <rx>, <tx>","-H","200","-L","10","-h","#dc322f","-l","#859900","-n","#286bd2"] 10,
        Run Date "%a %b %_d %l:%M" "date" 10,
        Run StdinReader
    ],
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %cpu%  %memory%  %battery%  %eth0%  <fc=#eee8d5>%date%</fc>  %EDDH%"
}
